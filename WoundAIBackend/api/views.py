...
# # from api.permissions import IsOwnerOrAdminOrDoctor, IsAdminOrDoctor, IsAdmin
...
#     PatientSerializer, WoundSerializer,
#     WoundImageCreateSerializer, WoundImageSerializer,
#     WoundAnalysisSerializer,
# )
# from .permissions import IsOwnerOrAdminOrDoctor, IsAdminOrDoctor
# from .utils import call_external_ai
...
#     except Patient.DoesNotExist:
#         return Response({"detail": "No patient profile linked"}, status=status.HTTP_404_NOT_FOUND)
...
#     def get_queryset(self):
#         u = self.request.user
#         if u.is_staff or u.role in ["admin", "doctor"]:
#             return Patient.objects.select_related("user").all()
#         return Patient.objects.filter(user=u).select_related("user")

#     def perform_create(self, serializer):
#         user_id = self.request.data.get("user_id")
#         if user_id:
#             if not (self.request.user.is_staff or self.request.user.role in ["admin", "doctor"]):
#                 raise PermissionError("Not permitted to create patient for another user")
...
#     queryset = Wound.objects.select_related("patient").all()
#     serializer_class = WoundSerializer
#     permission_classes = [IsAuthenticated, IsOwnerOrAdminOrDoctor]

#     def get_queryset(self):
#         u = self.request.user
#         qs = Wound.objects.select_related("patient", "patient__user")
#         if u.is_staff or u.role in ["admin", "doctor"]:
#             return qs
...
#         if not (u.is_staff or u.role in ["admin", "doctor"] or patient.user_id == u.id):
#             return Response({"detail": "Forbidden"}, status=status.HTTP_403_FORBIDDEN)
#         wounds = self.get_queryset().filter(patient=patient)
#         page = self.paginate_queryset(wounds)
#         ser = self.get_serializer(page or wounds, many=True)
#         if page is not None:
#             return self.get_paginated_response(ser.data)
#         return Response(ser.data)


# # ---------------- Wound Image (upload triggers AI call) ----------------
# class WoundImageViewSet(viewsets.ModelViewSet):
#     queryset = WoundImage.objects.select_related("wound", "wound__patient").all().order_by("-uploaded_at")
#     permission_classes = [IsAuthenticated, IsOwnerOrAdminOrDoctor]

#     def get_serializer_class(self):
#         if self.action == "create":
#             return WoundImageCreateSerializer
#         return WoundImageSerializer

#     def get_queryset(self):
#         u = self.request.user
#         qs = WoundImage.objects.select_related("wound", "wound__patient", "wound__patient__user")
#         if u.is_staff or u.role in ["admin", "doctor"]:
#             return qs
#         return qs.filter(wound__patient__user=u)
...
#         Steps:
#          - validate and save WoundImage (decoded image)
#          - call external AI service with image only
#          - create WoundAnalysis from AI response
#          - return created WoundImage serializer including analysis attached (if desired)
#         """
#         serializer = self.get_serializer(data=request.data)
#         serializer.is_valid(raise_exception=True)
#         wound_image = serializer.save(uploaded_by=request.user)

#         # Call external AI (only image and optional minimal meta)
#         try:
#             ai_payload_meta = {}  # per doc: not requiring additional manual clinical inputs
#             ai_response = call_external_ai(request.data.get("image_base64"), meta=ai_payload_meta)
#         except Exception as e:
#             # Big-picture: if AI fails, record rejection reason and continue
#             ai_response = {"is_wound": False, "rejection_reason": f"AI call failed: {str(e)}"}

#         # Map AI response to model fields safely
#         analysis_data = {
#             "wound_image": wound_image,
#             "is_wound": bool(ai_response.get("is_wound", False)),
#             "rejection_reason": ai_response.get("rejection_reason"),
#             "wound_area_cm2": ai_response.get("wound_area_cm2"),
#             "length_cm": ai_response.get("length_cm"),
#             "width_cm": ai_response.get("width_cm"),
#             "estimated_depth_mm": ai_response.get("estimated_depth_mm"),
#             "perimeter_cm": ai_response.get("perimeter_cm"),
#             "volume_cm3": ai_response.get("volume_cm3"),
#             "tissue_type": ai_response.get("tissue_type"),
#             "exudate": ai_response.get("exudate"),
#             "odor": ai_response.get("odor"),
#             "periwound_skin": ai_response.get("periwound_skin"),
#             "healing_stage": ai_response.get("healing_stage"),
#             "classification": ai_response.get("classification"),
#             "healing_index": ai_response.get("healing_index"),
#             "recommendations": ai_response.get("recommendations"),
#             "potential_complications": ai_response.get("potential_complications"),
#             "analysis_summary": ai_response.get("analysis_summary"),
#             "wound_outline_coordinates": ai_response.get("wound_outline_coordinates"),
#         }

#         WoundAnalysis.objects.create(**{k: v for k, v in analysis_data.items() if v is not None or k == "wound_image"})

#         read_ser = WoundImageSerializer(wound_image, context={"request": request})
#         return Response(read_ser.data, status=status.HTTP_201_CREATED)

#     @action(detail=True, methods=["GET"])
#     def analysis(self, request, pk=None):
#         wi = self.get_object()
#         if not hasattr(wi, "analysis"):
#             return Response({"detail": "Analysis not available yet"}, status=status.HTTP_202_ACCEPTED)
#         return Response(WoundAnalysisSerializer(wi.analysis).data)


# # ---------------- Wound Analysis (read-only) ----------------
# class WoundAnalysisViewSet(mixins.ListModelMixin, mixins.RetrieveModelMixin, viewsets.GenericViewSet):
#     queryset = WoundAnalysis.objects.select_related("wound_image__wound__patient").all().order_by("-analyzed_at")
#     serializer_class = WoundAnalysisSerializer
#     permission_classes = [IsAuthenticated, IsOwnerOrAdminOrDoctor]

#     def get_queryset(self):
#         u = self.request.user
#         qs = WoundAnalysis.objects.select_related("wound_image__wound__patient__user")
#         if u.is_staff or u.role in ["admin", "doctor"]:
#             return qs
#         return qs.filter(wound_image__wound__patient__user=u)
...
# from .serializers import ReportSerializer, ReportListSerializer, PrescriptionSerializer, PrescriptionCreateSerializer
# from .tasks import generate_prescription_ai_task

# # Simple helper permission checks (replace with your project's robust system)
# def is_admin(user):
...
#     queryset = Report.objects.all().select_related("created_by", "patient")
...
#     def get_queryset(self):
#         user = self.request.user
#         # Admin sees all in hospital; doctor sees own/assigned patients; patient sees own
#         if is_admin(user):
#             return Report.objects.filter(hospital_id=user.hospital_id).order_by("-created_at")
#         if is_doctor(user):
#             # doctors see reports for patients they manage or created (adjust according to your Patient model)
#             return Report.objects.filter(hospital_id=user.hospital_id).order_by("-created_at")
#         # patient
#         return Report.objects.filter(patient__user=user).order_by("-created_at")

#     def perform_create(self, serializer):
#         report = serializer.save(created_by=self.request.user)
#         AuditLog.objects.create(user=self.request.user, action="report_created", object_type="Report", object_id=str(report.id), details={"title": report.title})
#         return report

#     @action(detail=True, methods=["get"])
#     def share_link(self, request, pk=None):
#         """Return an expiring signed URL (frontend link) for sharing via native share sheet."""
#         report = self.get_object()
#         # Check permissions: allow if owner/doctor/admin or shared_with
#         user = request.user
#         allowed = False
#         if is_admin(user) or report.created_by == user or user in report.shared_with.all() or report.patient.user == user:
#             allowed = True
#         if not allowed:
#             return Response({"detail": "Forbidden"}, status=status.HTTP_403_FORBIDDEN)
...
#         payload = {
#             "report_id": report.id,
#             "exp": int((timezone.now() + timezone.timedelta(hours=48)).timestamp()),
#         }
#         token = jwt.encode(payload, settings.SECRET_KEY, algorithm="HS256")
#         signed_url = f"{settings.FRONTEND_URL.rstrip('/')}/reports/{report.id}/download?token={token}"
#         AuditLog.objects.create(user=user, action="report_share_link_generated", object_type="Report", object_id=str(report.id), details={"via": "share_link"})
#         return Response({"url": signed_url}, status=status.HTTP_200_OK)

#     @action(detail=True, methods=["post"])
#     def share(self, request, pk=None):
#         """Internal share to other system users (doctors). Accepts list of user ids."""
#         report = self.get_object()
#         user = request.user
#         # only owner/doctor/admin can share
#         if not (is_admin(user) or is_doctor(user) or report.created_by == user):
#             return Response({"detail": "Forbidden"}, status=status.HTTP_403_FORBIDDEN)
#         user_ids = request.data.get("share_with", [])
#         if not isinstance(user_ids, (list, tuple)):
#             return Response({"detail": "share_with must be a list of user IDs"}, status=status.HTTP_400_BAD_REQUEST)
#         # add to shared_with, and create audit log
#         added = []
#         for uid in user_ids:
#             try:
#                 u = get_user_model().objects.get(id=uid)
#                 report.shared_with.add(u)
#                 added.append(uid)
#             except Exception:
#                 continue
#         report.status = "shared"
#         report.save()
#         AuditLog.objects.create(user=user, action="report_shared", object_type="Report", object_id=str(report.id), details={"shared_with": added})
#         return Response({"success": True, "shared_with": added}, status=status.HTTP_200_OK)
...
# from .utils import generate_report_share_token
...
#     def share_link(self, request, pk=None):
#         """Generate a short-lived secure link for sharing"""
#         report = self.get_object()
#         token = generate_report_share_token(report, request.user)

#         share_url = f"https://wound-ai.aivolved.in/api/reports/{report.id}/download/?token={token}"
#         return Response({"share_url": share_url}, status=status.HTTP_200_OK)

#     @action(detail=True, methods=["get"], permission_classes=[AllowAny])
#     def download(self, request, pk=None):
#         """Download report with secure JWT token"""
...
#             report_id = validated_token.get("report_id")
#         except Exception:
#             return Response({"error": "Invalid or expired token"}, status=status.HTTP_401_UNAUTHORIZED)

#         if str(report_id) != str(pk):
#             return Response({"error": "Token does not match report"}, status=status.HTTP_403_FORBIDDEN)
...
#     queryset = Prescription.objects.all().select_related("requested_by", "patient", "wound")
#     authentication_classes = [TokenAuthentication]
#     permission_classes = [IsAuthenticated]
...
#         user = self.request.user
#         if is_admin(user):
#             return Prescription.objects.filter(hospital_id=user.hospital_id).order_by("-created_at")
#         if is_doctor(user):
#             # doctors see prescriptions they requested or for their patients
#             return Prescription.objects.filter(hospital_id=user.hospital_id).order_by("-created_at")
#         # patient sees only own prescriptions
#         return Prescription.objects.filter(patient__user=user).order_by("-created_at")

#     def perform_create(self, serializer):
#         pres = serializer.save(requested_by=self.request.user, hospital_id=self.request.user.hospital_id)
#         # kick off AI generation
#         generate_prescription_ai_task.delay(pres.id)
#         AuditLog.objects.create(user=self.request.user, action="prescription_requested", object_type="Prescription", object_id=str(pres.id), details={})
#         return pres

#     @action(detail=True, methods=["post"])
#     def approve(self, request, pk=None):
#         """Clinician reviews/approves AI suggestion."""
...
#         pres.clinician_review = review
#         pres.status = "approved"
#         pres.save()
#         AuditLog.objects.create(user=user, action="prescription_approved", object_type="Prescription", object_id=str(pres.id), details={"review": review})
#         return Response({"success": True, "prescription": PrescriptionSerializer(pres).data}, status=status.HTTP_200_OK)
...
#         pres.clinician_review = {"rejected_by": user.id, "reason": reason}
#         pres.save()
#         AuditLog.objects.create(user=user, action="prescription_rejected", object_type="Prescription", object_id=str(pres.id), details={"reason": reason})
#         return Response({"success": True}, status=status.HTTP_200_OK)

#     @action(detail=False, methods=["post"])
#     def generate(self, request):
#         """Alternative endpoint to explicitly ask to run AI generation for an existing Prescription."""
#         pres_id = request.data.get("prescription_id")
#         if not pres_id:
#             return Response({"detail": "prescription_id required"}, status=status.HTTP_400_BAD_REQUEST)
#         try:
#             pres = Prescription.objects.get(id=pres_id)
#         except Prescription.DoesNotExist:
#             return Response({"detail": "Not found"}, status=status.HTTP_404_NOT_FOUND)
#         generate_prescription_ai_task.delay(pres.id)
#         AuditLog.objects.create(user=request.user, action="prescription_generation_requested", object_type="Prescription", object_id=str(pres.id), details={})
#         return Response({"success": True, "task": "enqueued"}, status=status.HTTP_202_ACCEPTED)
...
@permission_classes([AllowAny])
def register(request):
    """Mobile signup: only patient/doctor. Patients must choose doctor(s)."""
    ser = UserCreateSerializer(data=request.data, context={"request": request})
    ser.is_valid(raise_exception=True)
    user = ser.save()
    token, _ = Token.objects.get_or_create(user=user)
    return Response(
...
# ---------------- Patient ----------------
class PatientViewSet(viewsets.ModelViewSet):
    queryset = Patient.objects.select_related("user").prefetch_related("doctors")
    serializer_class = PatientSerializer
    permission_classes = [IsAuthenticated, IsOwnerOrAdminOrDoctor]
    ...
    def get_queryset(self):
        ...
        if u.is_admin():
            return Patient.objects.select_related("user").all()
        if u.is_doctor():
            return Patient.objects.filter(doctors=u).select_related("user")
        return Patient.objects.filter(user=u).select_related("user")

    def perform_create(self, serializer):
        """Admins can create patient & assign doctors from backend; patients auto-linked to self."""
        if not (self.request.user.is_admin() or self.request.user.is_doctor()):
            serializer.save(user=self.request.user)
        else:
            serializer.save()
...
# ---------------- Wound ----------------
# class WoundViewSet(viewsets.ModelViewSet):
#     queryset = Wound.objects.select_related("patient", "patient__user")
#     serializer_class = WoundSerializer
#     permission_classes = [IsAuthenticated, IsOwnerOrAdminOrDoctor]
...
#         u = self.request.user
#         if u.is_admin():
#             return Wound.objects.all()
#         if u.is_doctor():
#             return Wound.objects.filter(patient__doctors=u)
#         return Wound.objects.filter(patient__user=u)

#     @action(detail=False, methods=["GET"], url_path="by-patient/(?P<patient_id>[^/.]+)")
#     def by_patient(self, request, patient_id=None):
#         patient = get_object_or_404(Patient, pk=patient_id)
#         if not (request.user.is_admin() or request.user in patient.doctors.all() or patient.user == request.user):
#             return Response({"detail": "Forbidden"}, status=status.HTTP_403_FORBIDDEN)
#         wounds = self.get_queryset().filter(patient=patient)
#         return Response(self.get_serializer(wounds, many=True).data)
...
#     def get_serializer_class(self):
#         return WoundImageCreateSerializer if self.action == "create" else WoundImageSerializer
...
#             return WoundImage.objects.all()
#         if u.is_doctor():
#             return WoundImage.objects.filter(wound__patient__doctors=u)
#         return WoundImage.objects.filter(wound__patient__user=u)

#     @transaction.atomic
#     def create(self, request, *args, **kwargs):
#         ser = self.get_serializer(data=request.data)
#         ser.is_valid(raise_exception=True)
#         wound_image = ser.save(uploaded_by=request.user)

#         # AI call
#         try:
#             ai_resp = call_external_ai(request.data.get("image_base64"), meta={})
#         except Exception as e:
#             ai_resp = {"is_wound": False, "rejection_reason": f"AI failed: {e}"}

#         WoundAnalysis.objects.create(
#             wound_image=wound_image,
#             **{k: v for k, v in ai_resp.items() if k in WoundAnalysisSerializer.Meta.fields}
#         )
#         return Response(WoundImageSerializer(wound_image, context={"request": request}).data, status=status.HTTP_201_CREATED)

#     @action(detail=True, methods=["GET"])
#     def analysis(self, request, pk=None):
#         wi = self.get_object()
#         if not hasattr(wi, "analysis"):
#             return Response({"detail": "Not ready"}, status=status.HTTP_202_ACCEPTED)
#         return Response(WoundAnalysisSerializer(wi.analysis).data)
...
class WoundAnalysisViewSet(mixins.ListModelMixin, mixins.RetrieveModelMixin, viewsets.GenericViewSet):
    ...
    def get_queryset(self):
        ...
        if u.is_admin():
            return WoundAnalysis.objects.all()
        if u.is_doctor():
            return WoundAnalysis.objects.filter(wound_image__wound__patient__doctors=u)
        return WoundAnalysis.objects.filter(wound_image__wound__patient__user=u)
...
from .serializers import WoundSerializer, WoundWithImagesSerializer, WoundImageSerializer, WoundImageCreateSerializer, WoundAnalysisSerializer
from .permissions import IsOwnerOrAdminOrDoctor
from rest_framework.permissions import IsAuthenticated

class WoundViewSet(viewsets.ModelViewSet):
    queryset = Wound.objects.select_related("patient", "patient__user").all()
    serializer_class = WoundSerializer
    permission_classes = [IsAuthenticated, IsOwnerOrAdminOrDoctor]
    ...
    def get_queryset(self):
        ...
        if u.is_admin():
            return Wound.objects.all()
        if u.is_doctor():
            # doctor sees wounds for patients they are assigned to
            return Wound.objects.filter(patient__doctors=u)
        # patient sees only own wounds
        return Wound.objects.filter(patient__user=u)

    @action(detail=False, methods=["POST"], url_path="create-with-images")
    def create_with_images(self, request):
        # only allow patient or doctor or admin to create on behalf
        serializer = WoundWithImagesSerializer(data=request.data, context={"request": request})
        serializer.is_valid(raise_exception=True)
        wound = serializer.save()
        return Response(WoundWithImagesSerializer(wound, context={"request": request}).data, status=status.HTTP_201_CREATED)

    @action(detail=False, methods=["GET"], url_path="by-patient/(?P<patient_id>[^/.]+)")
    def by_patient(self, request, patient_id=None):
        patient = get_object_or_404(Patient, pk=patient_id)
        user = request.user
        if not (user.is_admin() or (user.is_doctor() and user in patient.doctors.all()) or (patient.user == user)):
            return Response({"detail":"Forbidden"}, status=status.HTTP_403_FORBIDDEN)
        wounds = self.get_queryset().filter(patient=patient)
        return Response(self.get_serializer(wounds, many=True).data)
    ...
    def create(self, request, *args, **kwargs):
        ...
        if isinstance(data, list):
            for item in data:
                ...
                ser.is_valid(raise_exception=True)
                wi = ser.save(uploaded_by=request.user)
                # Dispatch ai analysis background task
                from .tasks import run_ai_analysis_for_image
                run_ai_analysis_for_image.delay(wi.id)
                created.append(wi)
            return Response(WoundImageSerializer(created, many=True, context={"request":request}).data, status=status.HTTP_201_CREATED)
        else:
            ...
            from .tasks import run_ai_analysis_for_image
            run_ai_analysis_for_image.delay(wi.id)
            return Response(WoundImageSerializer(wi, context={"request":request}).data, status=status.HTTP_201_CREATED)
...
def is_admin(user): return user.is_admin()
def is_doctor(user): return user.is_doctor()
def is_patient(user): return user.is_patient()
...
#         u = self.request.user
#         if u.is_admin():
#             return Report.objects.filter(hospital_id=u.hospital_id)
#         if u.is_doctor():
#             return Report.objects.filter(patient__doctors=u)
#         return Report.objects.filter(patient__user=u)

#     def perform_create(self, serializer):
#         report = serializer.save(created_by=self.request.user)
#         AuditLog.objects.create(user=self.request.user, action="report_created", entity="Report", entity_id=str(report.id))
#         return report

#     @action(detail=True, methods=["post"])
#     def share(self, request, pk=None):
#         """Share report with doctor(s)."""
#         report = self.get_object()
#         if not (request.user.is_admin() or request.user == report.patient.user):
#             return Response({"detail": "Forbidden"}, status=status.HTTP_403_FORBIDDEN)
#         doctor_ids = request.data.get("doctor_ids", [])
#         for did in doctor_ids:
#             try:
#                 d = get_object_or_404(User, id=did, role="doctor")
#                 report.shared_with.add(d)
#             except:
#                 continue
#         report.status = "shared"
#         report.save()
...
from .models import Report, AuditLog, User
...
class ReportViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        # ✅ No need to pass created_by (serializer handles it)
    ...
    def share(self, request, pk=None):
        """
        Share report with one or more doctors.
        Patients can share their own reports,
        ...
        if not isinstance(doctor_ids, list):
            return Response({"detail": "doctor_ids must be a list"}, status=status.HTTP_400_BAD_REQUEST)

        added, invalid = [], []
        for did in doctor_ids:
            doctor = User.objects.filter(id=did, role="doctor").first()
            if doctor:
        ...
        return Response(
            {"success": True, "shared_with": added, "invalid_ids": invalid},
            status=status.HTTP_200_OK,
        )
...
# ---------------- Prescriptions ----------------
class PrescriptionViewSet(viewsets.ModelViewSet):
    queryset = Prescription.objects.all().select_related("requested_by", "patient", "doctor")
    authentication_classes = [TokenAuthentication]
    permission_classes = [IsAuthenticated]
    ...
    def get_queryset(self):
        u = self.request.user
        if u.is_admin():
            return Prescription.objects.filter(hospital_id=u.hospital_id)
        if u.is_doctor():
            return Prescription.objects.filter(doctor=u)
        return Prescription.objects.filter(patient__user=u)

    # views.py
    def perform_create(self, serializer):
        pres = serializer.save()  # ✅ don’t pass requested_by again
        AuditLog.objects.create(
            user=self.request.user,
            action="prescription_requested",
            entity="Prescription",
            entity_id=str(pres.id),
        )
        return pres
    ...
    def create(self, request, *args, **kwargs):
        ...
        pres = serializer.instance  # the created Prescription
        output_serializer = PrescriptionSerializer(pres, context={"request": request})
    ...
    def approve(self, request, pk=None):
        pres = self.get_object()
        print(pres,">>>>>>>prez")
        if not (request.user.is_admin() or request.user == pres.doctor):
            return Response({"detail": "Forbidden"}, status=status.HTTP_403_FORBIDDEN)
        pres.status = "approved"
        pres.clinician_review = request.data.get("review", {})
        pres.save()
        return Response(PrescriptionSerializer(pres).data)
    ...
    def reject(self, request, pk=None):
        ...
        if not (request.user.is_admin() or request.user == pres.doctor):
            return Response({"detail": "Forbidden"}, status=status.HTTP_403_FORBIDDEN)
        pres.status = "rejected"
        pres.clinician_review = {"reason": request.data.get("reason", "")}
        pres.save()
        return Response({"success": True})
